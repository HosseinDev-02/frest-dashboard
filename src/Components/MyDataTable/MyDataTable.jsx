...
import {
    BiChevronDown,
    BiChevronUp, BiCog,
    BiCopy, BiDotsVerticalRounded,
    BiEdit,
    BiFile, BiMenu, BiPieChartAlt,
    BiPrinter,
    BiSolidFileExport,
    BiSolidFilePdf, BiTrash
} from "react-icons/bi";
...
export default function MyDataTable({ filteredUsers }) {
    ...
    const [columns, setColumns] = useState([
        {
            field: 'menu',
            headerName: 'عمل ها',
            cellClassName: 'mobile-action-column-css',
            headerClassName: 'mobile-action-header-column-css',
            renderCell: params => (
                <>
                    <MenuAction editHandler={editUserHandler(params.row)} deleteHandler={handleDeleteClick(params.id)} {...params}/>
                </>
            )
        },
        {
            ...
            renderCell: params => (
                <div className='flex items-center gap-4'>
                    ...
                    <div className='flex flex-col items-start'>
                        <span className='leading-6 text-2sm'>{params.row.firstName} {params.row.lastName}</span>
                        <span className='leading-6 text-xs text-muted'>{params.row.email}</span>
                    </div>
                </div>
            )
        },
        {
            ...
            renderCell: params => (
                <div className='flex items-center gap-2'>
                    <span
                        className={`flex items-center justify-center w-[30px] h-[30px] rounded-full ${params.row.role === 'نویسنده' ? 'bg-success/15 text-success' : params.row.role === 'ویرایشگر' ? 'bg-cyan/15 text-cyan' : 'bg-blue/15 text-blue'}`}>
                        {
        ...
        },
        {
            field: 'project',
            headerName: 'طرح',
            headerClassName: 'column-header-css-style column-header-project-css',
            cellClassName: 'column-css-style column-project-css',
        },
        {
            field: 'invoice',
            headerName: 'صورتحساب',
            headerClassName: 'column-header-css-style !basis-2/12 column-header-invoice-css',
            cellClassName: 'column-css-style !basis-2/12 !text-caption column-invoice-css',
        },
        {
            field: 'status',
            headerName: 'وضعیت',
            ...
            renderCell: (params) => (
                <span
                    className={`text-xs rounded px-2 ${params.row.status === 0 ? 'bg-orange/15 text-orange' : params.row.status === 1 ? 'bg-success/15 text-success' : 'bg-secondary/15 text-secondary'}`}>
                        {
                            params.row.status === 0 ? 'در انتظار' : params.row === 1 ? 'فعال' : 'غیرفعال'
                        }
                </span>
            )
        },
        {
            ...
            renderCell: (params) => (
                <div className='flex items-center gap-1'>
                    <span onClick={editUserHandler(params.row)} className='text-caption cursor-pointer'>
                        <BiEdit size='18px'/>
                    </span>
                    <span onClick={handleDeleteClick(params.id)} className='text-caption cursor-pointer'>
                        <BiTrash size='18px'/>
                    </span>
                </div>
            ),
    ...
    function tableSearchHandler(value) {
        if (value) {
            let filteredRows = users.filter(user => user.firstName.includes(value))
    ...
    const handlePrint = () => {
      ...
      <body>
        ...
        <table>
          <thead>
            <tr>
              ${columns.slice(0, 5).map(col => `<th>${col.headerName}</th>`).join("")}
            </tr>
          </thead>
          <tbody>
            ${rows
                  ...
                  ${columns.map(col => {
                    if (col.field !== 'actions') {
                        switch (col.field) {
                            ...
                            case 'status': {
                                return `<td>${row[col.field] === 0 ? 'در انتظار' : row[col.field] === 1 ? 'فعال' : 'غیرفعال'}</td>`
                            }
                            default:
    ...
    return (
        <div className='bg-white'>
            <div className='flex flex-col gap-4 md:flex-row items-center justify-between px-[22px] py-4'>
                <Input className='!w-auto' selectedItem={paginationModal.pageSize} setShowContent={setShowRowCounts}
                ...
                <div className='flex flex-col md:flex-row items-center gap-4'>
                    <Input className='!w-auto' inputClassName='!min-w-[200px]'
                           onChange={(event) => tableSearchHandler(event.target.value)} type='text'
                           placeholder='جستجو...'/>
                    <div className='flex items-center gap-4'>
                        <Input className='!w-auto !shrink-0'
                               renderButton={<SecondaryButton onClick={() => setShowExportTypes(prevState => !prevState)}
                                                              className='flex-row-reverse' icon='BiChevronDown'
                                                              iconSize='20px' title='برون بری'/>} selectedItem=''
                               setShowContent={setShowExportTypes} showContent={showExportTypes} type='selectbox'
                        ...
                        <PrimaryButton titleClassName='hidden md:inline' onClick={() => setShowAddNewUser(true)} className='!px-2' icon='BiPlus'
                                       title='افزودن کاربر جدید' iconSize='20px'/>
                    </div>
                    ...
                    <Content clickHandler={() => {
                    ...
                    }} title={mainUser ? 'ویرایش کاربر' : 'افزودن کاربر'} showContent={showAddNewUser} setShowContent={setShowAddNewUser}>
                        {/*  User FirstName Input  */}
                        <Input value={userFirstName} onChange={(event) => setUserFirstName(event.target.value)} type='text' label='نام' placeholder='جان'/>
                        {/*  User LastName Input  */}

                        <Input value={userLastName} onChange={(event) => setUserLastName(event.target.value)} type='text' label='نام خانوادگی' placeholder='اسنو'/>
                        {/*  User Email Input  */}

                        <Input value={userEmail} onChange={(event) => setUserEmail(event.target.value)} type='text' label='ایمیل' placeholder='example@gmail.com'/>
                        {/*  User Phone Input  */}

                        <Input type='text' label='تماس' placeholder='+989123456789'/>
                        {/*  User Company Input  */}

                        <Input type='text' label='شرکت' placeholder='توسعه دهنده وب'/>
                        {/*  User Country Input  */}

                        <Input label='کشور' selectedItem={selectedCountry} setShowContent={setShowUserCountry}
                               showContent={showUserCountry} type='selectbox'
                               placeholder={selectedCountry ? selectedCountry : 'انتخاب کشور'}>
                        ...
                        <Input label='نقش' selectedItem={selectedRole} setShowContent={setShowUserRoles}
                               showContent={showUserRoles} type='selectbox'
                               placeholder={selectedRole ? selectedRole : 'نقش کاربر'}>
                            <ul className='bg-white flex flex-col rounded-b overflow-y-auto max-h-64'>
                                {
                                    userRoles.map(role => (
                                        <li key={role.id} onClick={() => {
                                            ...
                                            className={`flex items-center gap-2 w-full h-9 px-3 transition-colors hover:bg-gray-50 cursor-pointer shrink-0`}>
                                            <span
                                                className='font-IranYekan-Medium text-2sm text-inherit'>{role.name}</span>
                                        </li>
                                    ))
                                }
                            </ul>
                        ...
                        <Input label='پلن' selectedItem={selectedPlan} setShowContent={setShowUserPlans}
                               showContent={showUserPlans} type='selectbox'
                               placeholder={selectedPlan ? selectedPlan : 'انتخاب پلن'}>
                            <ul className='bg-white flex flex-col rounded-b overflow-y-auto max-h-64'>
                                {
                                    userPlans.map(plan => (
                                        <li key={plan.id} onClick={() => {
                                            ...
                                            className={`flex items-center gap-2 w-full h-9 px-3 transition-colors hover:bg-gray-50 cursor-pointer shrink-0`}>
                                            <span
                                                className='font-IranYekan-Medium text-2sm text-inherit'>{plan.name}</span>
                                        </li>
                                    ))
                                }
                            </ul>
                        ...
                        <div className='flex gap-4'>
                            {
                                mainUser ? (
                                    <PrimaryButton onClick={editMainUser} title='ویرایش'/>
                                ) : (
                                    <PrimaryButton onClick={addNewUserHandler} title='افزودن'/>
                                )
                            }
                            <SecondaryButton onClick={() => setShowAddNewUser(false)} title='انصراف'/>
                        </div>
                    </Content>
                </div>
            </div>
