...
export default function Calendar() {
    ...
    const [eventLabels, setEventLabels] = useState([
        {id: 1, title: 'کسب و کار', color: 'rgb(90, 141, 238)', backgroundColor: 'rgba(90, 141, 238, 8%)'},
        {id: 2, title: 'شخصی', color: 'rgb(255, 91, 92)', backgroundColor: 'rgba(255, 91, 92, 8%)'},
        {id: 3, title: 'خانواده', color: 'rgb(253, 126, 20)', backgroundColor: 'rgba(253, 126, 20, 8%)'},
    ...
    const CustomButtons = ({direction, handleClick, disable}) => {
        return (
            <span onClick={handleClick}
                  className={`flex items-center justify-center rounded-full border border-blue text-blue w-6 h-6 ${disable ? 'cursor-default' : 'cursor-pointer'}`}>
                {
                    direction === 'left' ? (
    ...
    const isStartOfEvent = (day) => {
        return events.some((event) => +convertPersianNumbersToEnglish(day) === +event.start.day);
    };
        ...
        <div className='container'>
            <div className='flex flex-col-reverse lg:flex-row gap-y-6'>
                ...
                <div className='lg:basis-1/5'>
                    ...
                    {/*  Add Event Wrapper  */}
                    <Content clickHandler={() => setShowAddEventContent(false)} title='افزودن رویداد' showContent={showAddEventContent} setShowContent={setShowAddEventContent}>
                        <Input onChange={(event) => setEventTitle(event.target.value)} type='text' label='عنوان'
                               placeholder='عنوان رویداد'/>
                        {/* Event Label SelectBox */}
                        <Input type='selectbox' label='برچسب' showContent={showEventLabels}
                               setShowContent={setShowEventLabels} selectedItem={eventLabelSelected}>
                            <ul className='bg-white flex flex-col rounded-b overflow-hidden'>
                                {
                                    eventLabels.map(label => (
                                        <li onClick={() => {
                                            ...
                                            style={{ backgroundColor: label.title === eventLabelSelected.title ? label.backgroundColor : '' }}
                                            className={`flex items-center gap-2 w-full h-9 px-3 transition-colors ${label.title !== eventLabelSelected.title && 'hover:bg-gray-50'} cursor-pointer`}>
                                            <span style={{ backgroundColor: label.color }} className='rounded-full w-2.5 h-2.5'></span>
                                            <span
                                                className='font-IranYekan-Medium text-2sm text-inherit'>{label.title}</span>
                                        </li>
                                    ))
                                }
                            </ul>
                        ...
                        <div className='flex gap-4'>
                            <PrimaryButton onClick={addNewEvent} title='افزودن'/>
                    ...
                    <Box className='shadow-none rounded-none !h-auto text-center lg:text-right'>
                        ...
                        <div className='mt-6'>
                            <ul className='flex flex-col items-center lg:items-start justify-center lg:justify-start gap-4'>
                                ...
                                <li className='flex items-center gap-2'>
                                    <Input checked={checkedItems.has(1)} onChange={() => checkboxHandleCheck(1)}
                ...
                <div className='lg:basis-4/5 flex flex-col'>
                        ...
                        <div className='flex items-center justify-between gap-4 flex-wrap'>
                            ...
                            <div className='flex items-center gap-4'>
                                <div className='flex items-center gap-2'>
                                    <button className='text-title' onClick={calendarPreviousMonthHandler}>
                                        <BiChevronRight size='24px'/>
                                    </button>
                                    <button className='text-title' onClick={calendarNextMonthHandler}>
                    ...
                    <div className='bg-white w-full h-full'>
                        <table
                            className='w-full h-full text-2sm font-IranYekan-Medium table-fixed'>
                            <thead>
                                        ...
                                        <tr key={index}>
                                            {
                                                row.map((item, index) => {
                                                    return (
                                                        <td key={index} className='border-t border-r border-zinc'>
                                                            <div className='min-h-[100px] h-full'>
                                                                <div className='p-2'>
                                                                    <a href="#">
                                                                        {
                                                                            !item.disabled && item.day.jalali
                                                                        }
                                                                    </a>
                                                                </div>
                                                                {
                                                                    <div
                                                                        className={`${isStartOfEvent(item.day.jalali) && 'pr-1'} flex flex-col gap-1.5 relative`}>
                                                                        {getDayEvents(item.day.jalali).map((event, index) => {
                                                                            return (
                                                                                <div key={index} style={{
                                                                                        ...
                                                                                        isStartOfEvent(item.day.jalali) && (
                                                                                            <>
                                                                                             <span
                                                                                                 ...
                                                                                                 className={`absolute right-0 h-full w-[3px] bottom-0 top-0 rounded-r`}></span>
                                                                                                <span
                                                                                                    className='line-clamp-2'>{event.title}</span>
                                                                                            </>
                                                                                        )
                                                                                    }
                                                                                </div>
